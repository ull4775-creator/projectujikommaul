          ...
          <div class="col-md-6">
            ...
            {{method_field('PUT')}}
              ...
              <div class="card-body">
                    ...
                    <label for="pengguna">No Sambungan</label>
                    <select class="custom-select form-control-border" name="no_sa" id="no_sa">
                    @if(count($pengguna)>0)
                      @foreach($penggunas as $pengguna)
                        @if($pwngguna->no_sa == $inputs->no_sa)
                          <option selected value="{{$pengguna->no_sa}}">{{$pengguna->no_sa}}</option>
                        @else
                          <option value="{{$pengguna->no_sa}}">{{$pengguna->no_sa}}</option>
                        @endif
                      @endforeach
                    @else
                      <option disabled selected value="">-- Tidak ada pengguna --</option>
                    @endif
                    </select>
                  </div>
